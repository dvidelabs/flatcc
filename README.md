...
is kept separate.  in `builder.md`. Typical uses can be seen in the
`monster_test.c` file. The builder allows of repeated pushing of content
to a vector or a string while a containing table is being updated which
simplifies parsing of external formats. It is also possible to build
nested buffers in-line - at first this may sound excessive but it is
useful when wrapping a union of buffers in a network interface and it
ensures proper alignment of all buffer levels.
...
Because some use cases may be very constrained embedded devices, the
builder library can be customized with an allocator object and a buffer
emitter object. The separate emitter ensures a buffer can be constructed
without requiring a full buffer to be present in memory at once, if so
desired.
...
inspection alignment in buffers looks good so far. Load test to stress
internal buffer management on large inputs is still needed. Buffer
...
The portablity layer for older compilers is considered important, but it
...
That said, the current library is able encode and decode complex
flatbuffer structures as is, and there are no big known issues apart
from the above. The structure of the common header files are subject to
change, notably all functions and macros prefixed with `__`. The general
interface is subject to change based on feedback but it does appear
functional as is. The typeless flatcc_builder library should hopefully not
change much - it is mostly not user facing but other language exports
...
The compiler can optionally generally a single schema file with (-c) or
without common header for read only or for read and write (-w option). If a
schema includes other files, these are searched similar to `flatc` using
the -I directive and files are placed in the -o path if present.
The -r option will generate files for all included schema. The most
common option will be -a to generate all files.

Files can also be generated to stdout using the --stdout flag instead of -o
path. This can be used to generate a single compact header file for a specific
schema. The content is exactly the same as when generating individual files but
sorted by dependencies and concatenated. Dependent include files will not be
attempted include because their include guard flag is already defined in the
source. If -w or -a is selected, it is still necessary to link with the builder
library and include the associated header files.

The file sent to --stdout may appear rather large and this might raise concerns
about code bloat. However, most are inline functions that will never be
realized in the final object code. Old C compilers with poor optimization could
find it to be a handful though still far from the work required for C++ STL
templates.


## Endianness

The generated code supports the `FLATBUFFERS_LITTLEENDIAN` flag defined by the
`flatc` compiler and it can be used to force endianness. However, in most cases
it is better to rely on the detection provided, including the platform specific
`<endian.h>` when available.

It is best not to check the `FLATBUFFERS_LITTLEENDIAN` flag directly in compiled
C code. Instead it is recommended to use `flatbuffers_is_native_pe()`. It will
normally be a constant that optimizes out or worst case a runtime check that
also normally optimizes out when endianness cannot be determined at
preprocessing time. `pe` means protocol endian. The FlatBuffers protocol is
always Little Endian but it is possible to define a big endian protocol out of
the generated schema. For example, all structs have a `copy_from_pe` function
in the generated builder header and this may be used for non-flatbuffers
protocols in a network setting where structs are often big endian. (It may require
some hacks to change endianness, but only in a few places).
...
`builder.md`. Union table fields return a pointer of type
...
See also `builder.md` and `monster_test.c`.
...
cannot contain include statements - these will be ignored and may cause
errors when generating code due to absent definitions. When given as
file, the compiler essentially implements the command line interface for
`flatcc` but with more configurable options. The library also embeds the
builder library and it supports building binary schema files and buffers.

The library interface is defined in `flatcc.h` and follows the form:
create context, parse file or buffer, generate files, destroy context.
The parse is optional if only common files are generated. File
generation is optional if only syntax checks are required. See
`flatcc_cli.c` and `test_cgen.c` for more details.

The builder library is fairly small and must be linked with all
consumers of generated `..._builder.h files. The builder library may
also be used to create flatbuffers from scratch without generated code
or schema - including providing pre-generated vtables for high
performance applications, or to support buffer generation from
scripting languages.
