...
Especially fields named "identifier" have caused problems because the
field read accessor function coincideds with the autogenerated
`<table-name>_identifier` used as file identifier for buffers with the
given table as root. This can be solved by either renaming the
conflicting field name or by using the `-g` option:
