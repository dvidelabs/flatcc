...
foreach(LIBRARY IN LISTS WDK_LIBRARIES)
    #message(STATUS "UM library: " ${LIBRARY})
...
# Generate imported targets for WDK UM lib files
file(GLOB WDKUM_LIBRARIES "${WDK_ROOT}/Lib/${WDK_VERSION}/um/${WDK_PLATFORM}/*.lib")    
foreach(LIBRARY IN LISTS WDKUM_LIBRARIES)
    get_filename_component(LIBRARY_NAME ${LIBRARY} NAME_WE)
    string(TOUPPER ${LIBRARY_NAME} LIBRARY_NAME)
    if(NOT LIBRARY_NAME STREQUAL "WINDOWS")
        add_library(WDKUM::${LIBRARY_NAME} INTERFACE IMPORTED)
        set_property(TARGET WDKUM::${LIBRARY_NAME} PROPERTY INTERFACE_LINK_LIBRARIES  ${LIBRARY})
    endif()
endforeach(LIBRARY)
unset(WDKUM_LIBRARIES)
...
function(wdkum_add_library _target)
    cmake_parse_arguments(WDKUM "" "" "" ${ARGN})
    #message(STATUS "wdkum target: " ${_target})
    #message(STATUS "wdkum args: " ${WDKUM_UNPARSED_ARGUMENTS})
    add_library(${_target} ${WDKUM_UNPARSED_ARGUMENTS})

    set_target_properties(${_target} PROPERTIES COMPILE_OPTIONS "${WDKUM_COMPILE_FLAGS}")
    set_target_properties(${_target} PROPERTIES COMPILE_DEFINITIONS 
        "${WDK_COMPILE_DEFINITIONS};$<$<CONFIG:Debug>:${WDK_COMPILE_DEFINITIONS_DEBUG};_WIN32_WINNT=${WDK_WINVER}>"
        )
...
function(wdkum_add_app _target)
    cmake_parse_arguments(WDK "" "" "" ${ARGN})
